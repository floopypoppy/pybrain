...
class DataSet(Serializable):
    ...
    def batches(self, label, n, permutation=None):
        ...
        if permutation is not None:
            # indexes = [indexes[i] for i in permutation] # original
            for start, stop in indexes:
                ind = numpy.array(permutation[start:stop])
                yield self.data[label][ind]
        else:
            for start, stop in indexes:
                yield self.data[label][start:stop]

    # def batches(self, label_1, label_2, n, permutation=None):
    #     """Yield batches of the size of n from the dataset. 
    ...
    #     # Shuffle them according to the permutation if one is given
    #     mini_batch = SupervisedDataSet(self.indim, self.outdim)
    #     if permutation is not None:
    #         # indexes = [indexes[i] for i in permutation] # original
    #         for start, stop in indexes:
    #             ind = numpy.array(permutation[start:stop])
    #             mini_batch.setField('input', self.data[label_1][ind])
    #             mini_batch.setField('target', self.data[label_2][ind])
    #             yield mini_batch
    #     else:
    #         for start, stop in indexes:
    #             mini_batch.setField('input', self.data[label_1][start:stop])
    #             mini_batch.setField('target', self.data[label_2][start:stop])
    #             yield mini_batch

    # def batches(self, label, n, permutation=None):
    #     """(original) Yield batches of the size of n from the dataset.
    #     A single batch is an array of with dim columns and n rows. The last
    #     batch is possibly smaller.
    #     If permutation is given, batches are yielded in the corresponding
    #     order."""
    ...
    def randomBatches(self, label, n):
        ...
        # permutation = random.shuffle(range(len(self))) # original
        # full_batches, rest = divmod(len(self), n) # quotients and remainders
        # number_of_batches = full_batches if rest == 0 else full_batches + 1 #xuewen
        # permutation = range(number_of_batches) #xuewen
        permutation = range(len(self))
        random.shuffle(permutation) #xuewen
        return self.batches(label, n, permutation), permutation
