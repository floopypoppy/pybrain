...
class BackpropTrainer(Trainer):
    ...
    def train(self):
        ...
        self.module.resetDerivatives() # set all derivatives to 0
        errors = 0        
        ponderation = 0.
        shuffledSequences = []
        for seq in self.ds._provideSequences():
            shuffledSequences.append(seq)
        shuffle(shuffledSequences) # a random sequence
        for seq in shuffledSequences:
            e, p = self._calcDerivs(seq) # errors and ponderation; self.module.derivs would be changed
            errors += e
            ponderation += p
            if not self.batchlearning:
                gradient = self.module.derivs - self.weightdecay * self.module.params
                new = self.descent(gradient, errors) # ?errors seem not to be used
                if new is not None:
                    self.module.params[:] = new
                self.module.resetDerivatives() # if it's batch learning, self.module.derivs will accumulate 
                         					   # for each example

        # if self.verbose:
        #     print "Total error:", errors / ponderation
            #### total training errors averaged on each epoch
    ...
    def _calcDerivs(self, seq):
        """Calculate error function (forward implementation) and backpropagate output errors to yield 
        the gradient."""
        self.module.reset()        
        for sample in seq:
            self.module.activate(sample[0]) 
            # sample[0]:input;forward implementation with old params
            # self.module.outputbuffer is all updated
        error = 0
        ponderation = 0
        for offset, sample in reversed(list(enumerate(seq))):
            # need to make a distinction here between datasets containing
            # importance, and others
            target = sample[1] # sample[1]:target
            ...
            if len(sample) > 2:
                ...
                str(outerr)
                self.module.backActivate(outerr) # then self.module.derivs would be updated 
    ...
    def trainUntilConvergence(self, dataset=None, maxEpochs=None, verbose=None,
        ...
        bestweights = self.module.params.copy()   # randomly set
        bestverr = self.testOnData(validationData) # ponderated MSE of validation data
        trainingErrors = []
        ...
        while True:
            ...
            if verbose:
            	print 'train-errors: %.6f' %trainingErrors[-1]
            	print 'valid-errors: %.6f' %validationErrors[-1]
